...
const getApiKey = () => {
  ...
  if (!apiKey) {
    ...
    console.log('Using Grok API Key:', '***' + apiKey.slice(-4));
  }
  return apiKey;
};

// Initialize the Grok client with dynamic API key
function createGrokClient() {
  const apiKey = getApiKey();
  if (!apiKey) {
    throw new Error('Grok API key is required. Please set XAI_API_KEY or GROK_API_KEY environment variable.');
  }
...
export async function translateCodeWithGrok(sourceCode: string, sourceLanguage: string, targetLanguage: string): Promise<string> {
  try {
    console.log('Grok API Key:', process.env.XAI_API_KEY ? '***' + process.env.XAI_API_KEY.slice(-4) : 'Not found');
...
${sourceCode}`;
    ...
    console.error('Error in translateCodeWithGrok:', {
